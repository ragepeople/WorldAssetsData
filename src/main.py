...
from modules import logger, datetime, initEnvironment, initEngine, insert
...
class Portfolio:
  ...
  def __init__(self, engine, metadata_obj):
    ...
    logger.debug('Class Portfolio has been initialized')
  
  def doAuthorization(self, apiKey):
    ...
    if response.status_code == 200:
      ...
      logger.error(f"Status code isn't positive: {requests.status_code}")
  
  def getLimits(self, authToken):
    ...
    for key in jresponse.keys():
      if jresponse.get(f'{key}') != []:
        ...
        with self.engine.connect() as conn:
          table = self.metadata_obj.tables[f'{key}']
          result = conn.execute(insert(table), df_limits.to_dict("records"))
          conn.commit()
    

  def getPortfolio(self, authToken):
    ...
    if response.status_code == 200:
      ...
      with self.engine.connect() as conn:
        result = conn.execute(insert(self.metadata_obj.tables['portfolio']), jresponse)
        conn.commit()
    else:
...
# !!! Нужно доделать!
  def getDailySchedule(self, authToken):
  ...
  def getInstruments(self, authToken):
    ...
    for instrument in listTypes:
      ...
      while True:
        ...
        with self.engine.connect() as conn:
          result = conn.execute(insert(self.metadata_obj.tables['instruments']), df_instruments.where(df_instruments.notna(), other=None).to_dict("records"))
          conn.commit()
          # time.sleep(1)
  ...
  def getAllBids(self, authToken):
    ...
    while True: 
      ...
      with self.engine.connect() as conn:
        result = conn.execute(insert(self.metadata_obj.tables['allBids']), df_allbids.where(df_allbids.notna(), other=None).to_dict("records"))
        conn.commit()
  ...
  def searchAllDeals(self, authToken):
...
def main():  
  logger.info(f'SCRIPT HAS BEEN STARTED')
  DB_URL, API_KEY = initEnvironment()
  engine, metadata_obj = initEngine()
  
  portfolio = Portfolio(engine, metadata_obj)
  logger.info('Class portfolio has been initialized')
  
  logger.info('Starting to get an authKey')
  bearerToken = portfolio.doAuthorization(API_KEY)
  
  logger.info('Starting getLimits func')
  portfolio.getLimits(bearerToken)
  
  logger.info('Starting getPortfolio func')
  portfolio.getPortfolio(bearerToken)
  
  logger.info('Starting getInstruments func')
  portfolio.getInstruments(bearerToken)
  # getDailySchedule(bearerToken)
  
  logger.info('Starting getInstruments func')
  portfolio.getAllBids(bearerToken)
  # searchAllDeals(bearerToken)

  logger.info(f'SCRIPT HAS BEEN FINISHED')
